...
CREATE TABLE`ACCOUNTS_MAIN_TABLE` (
...
CREATE TABLE `CREDIT_CARD` (
  ...
  `SECURITY_CODE` TINYINT NULL,
  `PROVIDER` VARCHAR(255) NULL,
  `ACCOUNT_ID` INT NULL,
  PRIMARY KEY (`CREDIT_CARD_ID`),
   CONSTRAINT `fk_ACCOUNT_ID1`
    FOREIGN KEY (`ACCOUNT_ID`) REFERENCES `ACCOUNTS_MAIN_TABLE` (`ACCOUNT_ID`)ON DELETE NO ACTION ON UPDATE NO ACTION
    );
...
CREATE TABLE `CERTIFICATE_DEPOSIT_ACCOUNT` (
  ...
  `CERTIFICATE_DEPOSIT_ACCOUNT_ID` INT NOT NULL,
